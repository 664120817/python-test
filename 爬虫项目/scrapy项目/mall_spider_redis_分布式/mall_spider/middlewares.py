...
实现process_request方法
如果是请求是 https://cdnware.m.jd.com 开头的，就是设置一个iphone 的user-agent
...
class RandomUserAgent(object):
    ...
    def process_request(self,request,spider):
        #如果是请求是 https://cdnware.m.jd.com开头的，就是设置一个iphone的USER-AGENT
        if request.url.startswith('https://cdnware.m.jd.com'):
            request.headers['user-agent']='Mozilla/5.0 (iPhone; CPU iPhone OS 6_0 like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Version/6.0 Mobile/10A403 Safari/8536.25'
        else: #否则从user-agent中随机取出一个
...
2,实现process_request方法
 1，代理池中取出随机的代理IP，需要指定IP的协议，和访问的域名
 2，设置给request.meta['proxy]
 3,实现process_exception方法
   当请求出现异常的时候，代理池那些代理IP在本域名下是不可用的
"""
...
class ProxyMiddleware(object):
    ...
    # 实现process_request方法
    def process_request(self, request, spider):
        # 代理池中取出随机的代理IP，需要指定IP的协议，和访问的域名
