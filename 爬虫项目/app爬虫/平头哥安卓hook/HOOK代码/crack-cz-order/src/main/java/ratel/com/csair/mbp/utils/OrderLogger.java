package ratel.com.csair.mbp.utils;
...
public class OrderLogger {
    ...
    private static BlockingDeque<FileLogger.LogMessage> blockingDeque = new LinkedBlockingDeque<>(2048);
    ...
    public synchronized static void startRecord() {
        ...
        File baseDir = new File(RatelToolKit.whiteSdcardDirPath);
        RposedBridge.log("file start");
        File filename = new File(baseDir, "order_log.txt");
        mLogFile = filename;
    ...
    public static void outLog(final String message) {
        ...
        blockingDeque.offer(new FileLogger.LogMessage() {
            @Override
    ...
    public static void outLog(final FileLogger.LogMessage logMessage) {
        if (!isComponentStarted()) {
            return;
        }
        blockingDeque.offer(new FileLogger.LogMessage() {
