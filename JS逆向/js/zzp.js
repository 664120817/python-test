...
modulus = "96aed7cd4fc734caeb2ccc89f081ed9d4044b289f9e08387ec30e2d3893e63ef816682c72e90799637d85d19d7baf659c1631814a36738d95c576492f1b7bbf7";
setMaxDigits(parseInt(maxdigits, 10));
e = biFromHex(encryptionExponent);
m = biFromHex(modulus);
chunkSize = 2 * biHighIndex(m);
radix = 16;
barrett = new BarrettMu(m);
var charSum = 0;
...
while (encrypt.length % chunkSize !== 0) {
    encrypt[j++] = 0;
}
    ...
    block = new BigInt();
    j = 0;
for (var k = charCounter; k < charCounter+chunkSize; ++j) {
	block.digits[j] =[k++];
	block.digits[j] += [k++] << 8;
				}
     var crypt =barrett.powMod(block, e);
     var text = radix == 16 ? biToHex(crypt) : biToString(crypt, radix);
     encrypted += text + " ";
