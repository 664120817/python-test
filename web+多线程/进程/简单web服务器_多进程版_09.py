...
def main():
    ...
    while True:
        ...
        #创建进程对象
        handler_p=multiprocessing.Process(target=request_handler,args=(new_client_socket,ip_port))
        # 子线程守护主线程
        handler_p.daemon = True  # 如果主线程结束 子线程也结束
        handler_p.start()
...
def request_handler(new_client_socket,ip_port):
    ...
    print("欢迎客户端：{}上线".format(str(ip_port)))
    #判断协议是否为空
    if not request_data:
